...
def handle_affordance(obj, pt):
    ...
    def is_affordance(obj, pt):
        ...
        if (isinstance(obj, Trifold_Handle)):
            ...
            mesh_rotation = np.array([np.pi, 0, -np.pi / 2])
        ...
        elif (isinstance(obj, Ring_Handle)):
            _pt = apply_transformation(np.array([pt]), -np.array(obj.position), -np.array(obj.rotation), rotation_order='ZYX', offset_first=True)[0]
            ...
            mesh_position = np.array([0, obj.vertical_length[0], 0])
            mesh_rotation = np.array([0, np.pi / 2, 0])
            _pt = apply_transformation(np.array([_pt]), -mesh_position, -mesh_rotation, rotation_order='ZYX', offset_first=True)[0]

            _pt_radius = np.linalg.norm(_pt[[0, 2]])
            _pt_angle = np.arctan2(_pt[2], _pt[0])

            return (_pt_radius <= obj.vertical_separation[0]/2 + obj.mounting_radius[1] + AFFORDACE_PROXIMITY_THRES and 
                    _pt_radius >= obj.vertical_separation[0]/2 - obj.mounting_radius[1] - AFFORDACE_PROXIMITY_THRES and 
            ...
            mesh_position = np.array([0, obj.vertical_size[1], 0])
            ...
            mounting_radius = (obj.vertical_separation[0] - obj.vertical_size[0]) / 2

            return (_pt_radius <= outer_radius + AFFORDACE_PROXIMITY_THRES and 
                    _pt_radius >= mounting_radius - AFFORDACE_PROXIMITY_THRES and 
                    _pt[1] <= obj.vertical_size[0]/2 + AFFORDACE_PROXIMITY_THRES and 
                    _pt[1] >= -obj.vertical_size[0]/2 - AFFORDACE_PROXIMITY_THRES and 
                    _pt_angle >= np.pi/3 and _pt_angle <= np.pi*2/3)
